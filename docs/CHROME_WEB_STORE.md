...
- **アイコン**：リポジトリの `icons/`（128 / 48 / 16）。`python3 scripts/generate_extension_assets.py` で再生成可能。
- **プロモーション タイル（小）**：ストアの一覧や検索結果などで拡張機能を紹介するための横長バナー画像です（仕様はダッシュボード表示に従う）。本リポジトリでは **440×280 ピクセル、RGB PNG（アルファなし）** の `store-assets/promotional-tile-small-440x280.png` を用意しています。JPEG 必須の場合は画像アプリで書き出し直してください。
