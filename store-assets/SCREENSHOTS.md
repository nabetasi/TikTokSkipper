...
未加工のキャプチャを `store-assets/screenshots/source-screenshot.png` に置き（ファイル名は任意で、引数でパス指定も可）、次を実行すると、次の処理で **1280×800** と **640×400** の RGB PNG を出力します。

- **ぼかし**: **中央の動画コンテンツ帯**のみ強いガウスぼかし。左の **TikTok ロゴ・サイドメニュー**、右の **いいね等・拡張のボタン**は鮮明なまま（帯の境界は `scripts/process_store_screenshot.py` の `LEFT_SHARP_END_FRAC` / `RIGHT_SHARP_START_FRAC` で調整可能）。
- **リサイズ**: **クロップはしません**。アスペクト比を維持したままキャンバス全体に画像が収まるよう縮小・拡大し、足りない辺は **レターボックス**（余白）で埋めます。ロゴや右端 UI が切れないようにするための指定です。
